...
const matchResultSportsbook = async (bet365ApiKey) => {
    ...
    if (!bets || bets.length === 0) {
        console.log('no eligible SB bet');
        return;
    }

    for (const bet of bets) {
        try {
            const { lineQuery, pickName, pick } = bet;
            ...
            if (lineQuery.type.toLowerCase() == 'moneyline') {
                ...
                if (pick == 'away' || pick == 'under') linePoints = -linePoints;
            } else if (lineQuery.type.toLowerCase() == 'total') {
                linePoints = Number(linePoints[linePoints.length - 1]);
            }
            const { sportName, type, subtype } = lineQuery;
            const { data: { success, results } } = await axios
                .get(`https://api.b365api.com/v1/bet365/result`, {
                    params: {
                        token: bet365ApiKey,
            ...
            if (time_status == 3) { //Ended
                ...
                console.log(ss);
                if (ss == null || ss == "") {
                    await bet.update({ matchStartDate: new Date(Number(time) * 1000) });
                    continue;
                }
                const result = getMatchScores(sportName, type, subtype, ss, scores);
                if (result)
                    matchResult = { ...matchResult, ...result };
                else {
                    console.error("matchError:", eventId);
            ...
            if (cancellationReason) {
                ...
                const { _id, userId, bet: betAmount, toWin, pick } = bet;
                    ...
                    if (user) {
                        ...
                        const betChanges = {
                            $set: {
                                status: 'Settled - Win',
                                walletBeforeCredited: balance,
                                credited: toWin + betAmount,
                                homeScore,
                                awayScore,
                            }
                        }
                        const betFee = Number((toWin * 0.03).toFixed(2));
                        await Bet.findOneAndUpdate({ _id }, betChanges);
                        if (toWin > 0) {
                            await User.findOneAndUpdate({ _id: userId }, { $inc: { balance: betAmount + toWin - betFee } });
                            await FinancialLog.create({
                                financialtype: 'betwon',
                                uniqid: `BW${ID()}`,
                                user: userId,
                                amount: betAmount + toWin,
                                method: 'betwon',
                                status: FinancialStatus.success,
                            });
                            await FinancialLog.create({
                        ...
                        if (!preference || !preference.notification_settings || preference.notification_settings.win_confirmation.email) {
                            const msg = {
                                ...
                                to: email,
                                subject: 'You won a wager!',
                                text: `Congratulations! You won $${toWin.toFixed(2)}. View Result Details: https://www.payperwin.co/history`,
                                html: simpleresponsive(`
                                        <p>
                                            Congratulations! You won $${toWin.toFixed(2)}. View Result Details:
                                        </p>
                                        `,
                                    { href: 'https://www.payperwin.co/history', name: 'View Settled Bets' }
                                ),
                        ...
                        if (user.roles.phone_verified && (!preference || !preference.notification_settings || preference.notification_settings.win_confirmation.sms)) {
                            sendSMS(`Congratulations! You won $${toWin.toFixed(2)}.`, user.phone);
                        }
                    }
                } else if (betWin === false) {
                    const betChanges = {
    ...
    console.log('Finished checking SB Bets', new Date().toLocaleString());
}
...
const matchResults = async () => {
    ...
    matchResultSportsbook(bet365ApiKey);
}
